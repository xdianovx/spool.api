            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <form id="formAccountSettings" method="POST" action="{{ route('video.update', $video->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="text" id="name" name="name" placeholder="Введите название" value="{{ $video->name }}" required>
                                @error('name')
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select id="partner_company" class="select2 form-select" name="partners_company_id">
                                    ...
                                    <option value="{{ $partner_company->id }}" {{ $partner_company->id == $partner_company->partners_company_id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                @endif
                                <div class="input-group">
                                    <input type="file" class="form-control" id="image_banner" name="image_banner" value="{{ $video->image_banner }}">
                                </div>
                                @error('image_banner')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="text" id="video" name="video" placeholder="Введите ссылку на видео" value="{{ $video->video }}" required>
                                @error('video')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <textarea id="basic-default-message" class="form-control" name="description" placeholder="Текст" style="height: 234px;" required>{{ $video->description }}</textarea>
                                @error('description')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="text" id="duration" name="duration" placeholder="Ввидите длительность видео" value="{{ $video->duration }}" required>
                                </input>
                                @error('duration')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="data" id="event_date" name="event_date" placeholder="Введите дату события" value="{{ $video->event_date }}" required>
                                </input>
                                @error('event_date')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="text" id="minimum_age" name="minimum_age" placeholder="Введите минимальный возраст" value="{{ $video->minimum_age }}" required>
                                </input>
                                @error('minimum_age')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <div class="form-check mt-3">
                                    <input name="display_slider" class="form-check-input" type="radio" value="false" id="defaultRadio1" @if ($video->display_slider == 'false') checked="checked" @else @endif>
                                    <label class="form-check-label" for="defaultRadio1"> Нет </label>
                                </div>
                                <div class="form-check">
                                    <input name="display_slider" class="form-check-input" type="radio" value="true" id="defaultRadio2" @if ($video->display_slider == 'true') checked="checked" @else @endif>
                    ...
                    <div class="card-body">
                        ...
                        <form id="formAccountSettings" method="POST" action="{{ route('tag.store', $video->id) }}">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3 col-md-6 d-flex gap-2">
                                    ...
                                    <input class="form-control" type="text" id="firstName" name="name" placeholder="Введите название" value="{{ old('name') }}" required>
                                    @error('name')
                                    <div class="text-danger">{{ $message }}</div>
                                    @enderror
                        ...
                        <table class="table tag_table" data-video-id="{{$video->id}}">
                            <thead>
                                <tr>
                                    <th>Название</th>
                                    <th>Показать?</th>
...
<script>
    ...
    const videoId = document.querySelector('.tag_table').getAttribute('data-video-id');

    let data
    let tagWrap

    const listUpdate = async () => {
        await fetch('/tags/'+videoId)
            .then(res => res.json())
