...
@section('content')
    <div class="container-xxl flegrow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light"><a href="{{ route('tickets.index',) }}">
            Билеты</a> / {{$ticket->id}} / </span>Редактировать</h4>
            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <form id="formAccountSettings" method="POST" action="{{ route('ticket.update', $ticket->id) }}">
                            @csrf
                            @method('patch')
                            <div class="row">
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input class="form-control" type="text" id="firstDiscountedPrice" name="discounted_price"
                                        value="{{ $ticket->discounted_price }}" autofocus autocomplete="discounted_price">
                                    @error('discounted_price')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label for="video" class="form-label">Партнерская компания</label>
                                    @if (!count($videos) == 0)
                                    <select id="video" class="select2 form-select" name="video_id">
                                        @foreach ($videos as $video)
                                        <option value="{{ $video->id }}" {{ $video->id == $ticket->video_id ? 'selected' : '' }}>
                                            {{ $video->name }}
                                        </option>
                                        @endforeach
                                    </select>
